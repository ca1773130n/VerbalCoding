...
5. **Seed SOUL.md from project context** — Wizard writes the wizard's `AGENT_PROJECT_CONTEXT` answer into `<profile>/SOUL.md` as a marker-delimited block (`<!-- vc:project-context:start -->` … `<!-- vc:project-context:end -->`). When `--clone` carries Hermes' default persona into SOUL.md, the wizard appends the project block below it. Re-running setup is idempotent: the helper replaces the existing marker block in place, so a different project context overwrites the old one rather than stacking duplicates. Refresh fires on both create and reuse paths so re-running setup with a new project context updates an existing profile.
